...
<body>
	...
	<table class="table table-condensed">
		<tr><td>1</td><td>Nome</td>
			<td><?php
				$nome = $_POST["nome"];//pega o quer foi digitado no campo nome
				if(!$nome){ //verifica se foi digitado?>
				<td class="active"> <?php echo "Campo não informado";?></td><?php 
			}else {
				if(preg_match('/^[A-Z]{1,}[a-z][^\d]{1,}$/', $nome))//verifica se o nome será valido na expressão regualar
					{ ?>
...
<tr><td>2</td><td>CPF</td>
	<td><?php
					$cpf = $_POST["cpf"];// 11 digitos
					if(!$cpf){ ?>
					<td class="active"> <?php echo "Campo não informado";?></td><?php
				}else {
					if(preg_match('/^[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}$/', $cpf))// \. define que o ponto deve ser informado
						{ ?>
	...
	<tr><td>3</td><td>RG</td>
		<td><?php
						$rg = $_POST["rg"];// 9 digitos
						if(!$rg){ ?>
						<td class="active"> <?php echo "Campo não informado";?></td><?php
					}else {
						if(preg_match('/^[0-9]{1}[0-9a-z]{8}$/', $rg))//inicia com 1 numero seguido de 8 digitos nº ou letras
							{ ?>
		...
		<tr><td>4</td><td>Endereço</td>
			<td><?php
				...
				if(preg_match('/^[a-zA-Z ]{1,}\ [0-9]{1,}\ [0-9a-zA-Z ]{1,}/', $endereco))// verifica se endereco é valida
					{ ?>
...
<tr><td>6</td><td>Data</td>
	<td><?php 
		...
		<td class="active"> <?php echo "Campo não informado";?></td><?php
			...
			if($diferenca < 0 ) { // verifica se data é maior que a data atual
				?>
				<td class="danger"> <?php echo "A data $dangerta é maior que data atual $dataAtual";?></td><?php
			} else {	
				if(preg_match('/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/', $data)) // verifica se é valida na expressão
					{ ?>
